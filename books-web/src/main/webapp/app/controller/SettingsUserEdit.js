...
App.controller('SettingsUserEdit', function($scope, $state, $stateParams, Restangular) {
   ...
   */
  if ($scope.isEdit()) {
    Restangular.one('admin', $stateParams.username).get().then(function(data) {
      $scope.user = data;
    });
  }
  ...
  $scope.edit = function() {
    ...
    if ($scope.isEdit()) {
      promise = Restangular
      .one('admin', $stateParams.username)
      .post('', $scope.user);
    } else {
      promise = Restangular
      .one('admin')
  ...
  $scope.remove = function () {
    if (confirm('Do you really want to delete this user? All associated documents, files and tags will be deleted')) {
      Restangular.one('admin', $stateParams.username).remove().then(function() {
        $scope.loadUsers();
        $state.transitionTo('settings.user');
      }, function () {
        $state.transitionTo('settings.user');
